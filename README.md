...
3. **Импорт данных из файлов Excel (.xls, .xlsx)**
4. **Программный интерфейс для работы с заявками**
...
- **Язык программирования**: Python (или другой на усмотрение разработчика).
- **СУБД**: PostgreSQL / MySQL / SQLite (на выбор).
- **Формат данных**: Импорт данных из файлов Excel (.xls, .xlsx).

## Архитектура системы

### 1. **Frontend**:
   - Простой интерфейс для ввода заявок, изменения статуса и назначения мастеров.
   - Визуализация статистики.

### 2. **Backend**:
   - Логика обработки заявок, расчет статистики.
   - Взаимодействие с базой данных для хранения и обработки информации.

### 3. **База данных**:
   - Таблица для заявок (с полями: ID, дата, вид техники, модель, описание проблемы, ФИО клиента, телефон, статус и т.д.).
...
Для удобства использования система поддерживает импорт данных о заявках из файлов Excel (.xls, .xlsx). В процессе импорта данные приводятся к нормальной форме базы данных для обеспечения целостности и консистентности.
